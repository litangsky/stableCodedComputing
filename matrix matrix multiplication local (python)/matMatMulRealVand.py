# -*- coding: utf-8 -*-
##########################################################################################
# Code for real Vandemonde method (real polynomial codes)
...
from matMatMul import addGaussianNoise, getRealVandEncodingMatOfA, getRealVandEncodingMatOfB, getEncodingMat, randMatGenerator, getMultResult, encodeMat, pickWorkers, transmitSubMat2Workers, computeWorker, reshapeReceivedInfo, decodeResult, getMSE, findWorkerSet2WorstCond

#########################################################
#matrix A (dim t*(kA*bA)) and matrix B (dim t*(kA*bA))
n = 31 # number of total workers 
kA = 4  # 1/uA storage fraction of A
kB = 7 # 1/uB storage fraction of B
